...
export class NuevoproveedorComponent implements OnInit{
  ...
  constructor(private provedorServicio:ProveedorService, private navegacion:Router, private ruta:ActivatedRoute){}
  ngOnInit(): void {
    this.idProveedores = parseInt(this.ruta.snapshot.paramMap.get('id'));

    if(this.idProveedores > 0){
      this.provedorServicio
  ...
  grabar(){
    ...
    this.provedorServicio.insertar(iproveedor).subscribe(
      (respuesta) => {
      ...
      this.provedorServicio.actualizar(iproveedor).subscribe(
        (respuesta) => {
